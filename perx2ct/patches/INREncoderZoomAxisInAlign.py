...
class INREncoderZoomAxisInAlign(nn.Module):
    print("Model Name : INREncoderZoomAxisInAlign - Use local feature with NeRF(include DummyNeRF)")
    ...
    def rendering_from_ctslice(self, p0, zoom_size, ct_slice, output_ct_res):
        ...
        p1 = [p / (res-1) * 2 - 1 for p, res in zip(p1, input_ct_res)]
    ...
    def get_rays_for_no_rendering(self, inputs: dict, p0, zoom_size):
        ...
        transformed_points_in_gt = torch.meshgrid(torch.linspace(0, self.ct_res[0] - 1, sampling_resolution[0]),
        ...
        transformed_points_in_gt = transformed_points_in_gt.unsqueeze(0).to(inputs[inputs['image_key']].device)
        ...
        if self.axis_emb.weight.device != device:
    ...
    def run_nerf(self, org_shape, transformed_points, nrays_grad_on, latent_zs=None, full_render_partial_grad=False):
    ...
    def forward(self, inputs: dict, full_render_partial_grad=False, gt_ct=None, p0=None, zoom_size=None):
        ...
        if self.use_cond_encoder:
            ...
            if self.no_grad_encoder:
                with torch.no_grad():
                    latent_zs = self.network_fn.encode(src_imgs, src_camposes, transformed_points[..., :3])
            else:
                latent_zs = self.network_fn.encode(src_imgs, src_camposes, transformed_points[..., :3])
        else:
            latent_zs = None
        ...
        if gt_ct is not None:
            ...
            all_outputs = self.run_nerf((batch_size, H, W, N_samples), transformed_points, nrays_grad_on,
                                        latent_zs, full_render_partial_grad=full_render_partial_grad)
